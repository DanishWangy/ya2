...
let handler = async (m, { conn, usedPrefix: _p, args, command }) => {
  ...
  try {
    ...
    if (teks == '404') {
      return conn.relayWAMessage(conn.prepareMessageFromContent(m.chat, {
        "listMessage": {
          ...
          "description": "Berikut ini adalah daftar menu DaniBotz.",
          "footerText": "Silahkan tekan tombol \"Click Here\" untuk melihat sub-menu DaniBotz.\n\nJika menemukan bug, error atau kesulitan dalam penggunaan silahkan laporkan/tanyakan kepada owner.",
    ...
    await conn.send3ButtonLoc(m.chat, await (await fetch(thumbfoto)).buffer(), text.trim(), watermark, 'Pemilik Bot', `${_p}owner`, 'Donasi', `.donasi`, 'Group Official', '.harunoff',  m)
  } catch (e) {
